        ...
        try (FileWriter writer = new FileWriter(file)) {
            for (String key : keys) {
                ...
                if (key.equalsIgnoreCase(keys.last())) {
                    writer.write(String.format("%s(%s, %s);", key.replace("minecraft:", "").toUpperCase(Locale.ROOT), id,
                        Registries.BLOCKS.stream().map(Block::getIdentifier).collect(Collectors.toList()).contains(key)));
                } else {
                    writer.write(String.format("%s(%s, %s),", key.replace("minecraft:", "").toUpperCase(Locale.ROOT), id,
                        Registries.BLOCKS.stream().map(Block::getIdentifier).collect(Collectors.toList()).contains(key)));
                    writer.write("\n");
